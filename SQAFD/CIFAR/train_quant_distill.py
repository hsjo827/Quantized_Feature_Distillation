...
parser.add_argument('--lr_c', type=float, default=1e-6, help='learning rate for adapter parameters')
...
parser.add_argument('--fd_type', type=str, default='L2', choices=['L1', 'L2'], help='Feature Distillation Loss type, L1(MAE) or L2(MSE)')
...
if args.quan_method == "EWGS" or args.baseline:
    ...
    for m in model.modules():
        if args.use_adapter and m in adapter_layers_s:
            continue
...
if args.distill:
    ...
    for m in model_t.modules():
        if args.train_last_conv and m in last_conv_layers_t:
            continue
    ...
    if total_params_t != (model_params_count_t + quant_params_count_t + last_conv_params_count_t):
        raise Exception('Mismatched number of trainable parmas')
...
if args.use_adapter:
    if args.optimizer_a == 'SGD':
        optimizer_a = torch.optim.SGD(adapter_params_s, lr=args.lr_a, momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.optimizer_a == 'Adam':
        optimizer_a = torch.optim.Adam(adapter_params_s, lr=args.lr_a, weight_decay=args.weight_decay)
    ...
    if args.lr_scheduler_a == "step":
        ...
        scheduler_a = torch.optim.lr_scheduler.MultiStepLR(optimizer_a, milestones=milestones_a, gamma=args.gamma)
    elif args.lr_scheduler_a == "cosine":
        scheduler_a = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_a, T_max=args.epochs, eta_min=args.lr_a_end)
...
for ep in range(args.epochs):
    if args.distill:
        ...
        # set teacher as eval(), when train teacher's last conv layer set teacher as train()
        if not args.train_last_conv:
            module_list[-1].eval()
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.distill:
            ...
            feat_s, block_out_s, logit_s, quant_params, fd_map_s = model_s(
                images, save_dict, lambda_dict, is_feat=True,
                preact=preact, flatGroupOut=flatGroupOut
            )

            # forward teacher model
            # train conv2d, batchnorm2d + freeze batchnorm2d statistics
            if args.train_last_conv:
                for param in last_conv_params_t:
                    param.requires_grad = True

                for layer in last_conv_layers_t:
                    if isinstance(layer, nn.BatchNorm2d):
                        layer.eval()

                feat_t, block_out_t, logit_t, fd_map_t = model_t(
                    images, is_feat=True, preact=preact,
                    flatGroupOut=flatGroupOut, quant_params=quant_params
                )
        ...
        if args.distill:
            ...
            loss_div_t = criterion_div(logit_s.detach(), logit_t) # KL for last conv
            if args.distill == "crdst":
                ...
                loss_kd = criterion_kd(fd_map_s, fd_map_t) # L1(MAE) or L2(MSE)
                loss_total = args.kd_gamma * loss_div + args.kd_alpha * loss_kd # SQAFD Loss
                
                # track loss
                loss_cls_value = loss_cls.item()
                loss_kd_value = loss_kd.item() if args.distill != "crdst" else 0
        ...
        loss = loss_total
        loss.backward(retain_graph=args.train_last_conv)
