...
class FeatureQuantizer(nn.Module):
    def __init__(self, num_levels, scaling_factor, baseline=False):
        ...
        # Upper and lower bounds as learnable parameters
        self.uF = nn.Parameter(torch.tensor(0.0).float()) 
        self.lF = nn.Parameter(torch.tensor(0.0).float())  
        ...
        self.output_scale = nn.Parameter(torch.tensor(1).float()) 
