...
def update_grad_scales(model, model_t, train_loader, criterion, device, args): # 수정
    ## update scales
    if args.QActFlag:
        scaleA = []
    if args.QWeightFlag:
    ...
    if args.QFeatureFlag and hasattr(model_t, 'feature_quantizer'): # 수정
        model_t.feature_quantizer.hook_feature_values = True
        scaleF.append(0)  
    ...
    with tqdm(total=3, file=sys.stdout) as pbar:
        for num_batches, (images, labels) in enumerate(train_loader):
            ...
            if args.QFeatureFlag and hasattr(model_t, 'feature_quantizer'): # 수정
                Qfeature = model_t.feature_quantizer.buff_feature
            ...
            # Update scaling factors for features
            if args.QFeatureFlag and Qfeature is not None: # 수정
                trace_hess_F = np.mean(trace(model_t, [Qfeature], [Qfeature.grad], device))
                avg_trace_hess_F = trace_hess_F / Qfeature.view(-1).size()[0]
                scaleF[0] += avg_trace_hess_F / (Qfeature.grad.std().cpu().item() * 3.0)

            pbar.update(1)
    
    # Normalize and clip scaling factors (수정)
    if args.QWeightFlag:
        scaleW = [np.clip(s / num_batches, 0, np.inf) for s in scaleW]
    if args.QActFlag:
        scaleA = [np.clip(s / num_batches, 0, np.inf) for s in scaleA]
    if args.QFeatureFlag:
        scaleF = [np.clip(s / num_batches, 0, np.inf) for s in scaleF]
    ...
    # Update model_t parameters for FeatureQuantizer (수정)
    if args.QFeatureFlag and hasattr(model_t, 'feature_quantizer'):
        model_t.feature_quantizer.bkwd_scaling_factorF.data.fill_(scaleF[0])
        model_t.feature_quantizer.hook_feature_values = False
    ...
    # Log results for debugging (수정)
    if args.QWeightFlag:
        print("\nUpdated weight scaling factors:", scaleW)
    if args.QActFlag:
        print("Updated activation scaling factors:", scaleA)
    if args.QFeatureFlag:
        print("Updated feature scaling factors:", scaleF)
