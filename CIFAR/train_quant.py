...
parser.add_argument('--QFeatureFlag', type=str2bool, default=True, help='do activation quantization') # 수정
...
parser.add_argument('--bkwd_scaling_factorW', type=float, default=0, help='scaling factor for weights')
parser.add_argument('--bkwd_scaling_factorA', type=float, default=0,  help='scaling factor for activations')
parser.add_argument('--bkwd_scaling_factorF', type=float, default=0, help='scaling factor for teacher activations') 
...
if args.quan_method == "EWGS" or args.baseline:
    ...
    if args.distill: 
        ...
        # 수정
        teacher_quant_params = []
        if args.QFeatureFlag:
            model_t.feature_quantizer = FeatureQuantizer(
                num_levels=args.act_levels,
                scaling_factor=args.bkwd_scaling_factorF,
                baseline=args.baseline
            )
            teacher_quant_params.append(model_t.feature_quantizer.uF)
            teacher_quant_params.append(model_t.feature_quantizer.lF)
            teacher_quant_params.append(model_t.feature_quantizer.output_scale)
 

    # 수정
    if args.QFeatureFlag:
        trainable_params += teacher_quant_params 
        print("# teacher quantizer params:", sum(p.numel() for p in teacher_quant_params))
        logging.info("# teacher quantizer params: {}".format(sum(p.numel() for p in teacher_quant_params)))
    ...
    if sum(p.numel() for p in trainable_params) != sum(p.numel() for p in model_params) + sum(p.numel() for p in quant_params) + (sum(p.numel() for p in teacher_quant_params) if args.QFeatureFlag else 0):
...
if define_quantizer_scheduler:
    ...
    # 수정
    if args.distill and args.QFeatureFlag: 
        if args.optimizer_q == 'SGD':
            optimizer_teacher_q = torch.optim.SGD(teacher_quant_params, lr=args.lr_q)
        elif args.optimizer_q == 'Adam':
            optimizer_teacher_q = torch.optim.Adam(teacher_quant_params, lr=args.lr_q)
    ...
    if args.lr_scheduler_q == "step":
        ...
        if args.distill and args.QFeatureFlag: # 수정
            scheduler_teacher_q = torch.optim.lr_scheduler.MultiStepLR(optimizer_teacher_q, milestones=milestones_q, gamma=args.gamma)
        ...
        if args.distill and args.QFeatureFlag:  # 수정
            scheduler_teacher_q = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_teacher_q, T_max=args.epochs, eta_min=args.lr_q_end)
...
for ep in range(args.epochs): 
    ...
    if ep % args.update_every == 0 and ep != 0 and not args.baseline and args.use_hessian:
        update_grad_scales(model, model_t, train_loader, criterion, device, args)
    ...
    if define_quantizer_scheduler:
        ...
        if args.QFeatureFlag: # 수정
            writer.add_scalar('train/teacher_quant_lr', optimizer_teacher_q.param_groups[0]['lr'], ep)
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.distill:
            ...
            if args.distill in ['abound']:
                preact = True 
            # 수정 (pool_out_s)
            feat_s, block_out_s, pool_out_s, logit_s = model_s(images, save_dict, lambda_dict, is_feat=True, preact=preact, flatGroupOut=flatGroupOut)
            with torch.no_grad():
                # 수정 (pool_out_t)
                feat_t, block_out_t, pool_out_t, logit_t = model_t(images, is_feat=True, preact=preact, flatGroupOut=flatGroupOut) 
                feat_t = [f.detach() for f in feat_t]

            # 수정 (pool_out_t에만 FeatureQuantizer 적용)
            if args.QFeatureFlag:
                pool_out_t = model_t.feature_quantizer(pool_out_t)
        ...
        if args.distill:
            ...
            # 수정
            if args.QFeatureFlag: 
                loss_feature = criterion_kd(pool_out_s, pool_out_t)
                loss_total = args.kd_gamma * loss_cls + (1-args.kd_gamma) * loss_feature
                ...
                writer.add_scalar('train/loss_feature', loss_feature_value, total_iter)
                writer.add_scalar('train/loss_total', loss_total_value, total_iter)
                content = f"total_iter={total_iter}, loss_cls={loss_cls_value}, loss_feature={loss_feature_value}, loss_total={loss_total_value}"
            ...
            if i == 0:
                if args.QFeatureFlag:
                    printRed(f"gamma: {args.kd_gamma}, loss_feature_weight: {1 - args.kd_gamma}")
                else:
    ...
    with torch.no_grad():
        ...
        if args.QFeatureFlag:
            checkpoint.update({
                'optimizer_teacher_q': optimizer_teacher_q.state_dict(),
                'scheduler_teacher_q': scheduler_teacher_q.state_dict(),
                'kd_gamma': args.kd_gamma
            })

        torch.save(checkpoint, os.path.join(args.log_dir, 'checkpoint/last_checkpoint.pth')) 

        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(checkpoint, os.path.join(args.log_dir, 'checkpoint/best_checkpoint.pth'))
    ...
    if args.QFeatureFlag and hasattr(model_t, 'feature_quantizer'):
        # Log teacher's FeatureQuantizer parameters to log.txt
        feature_quantizer_params = {
            "lF": model_t.feature_quantizer.lF.item(),
            "uF": model_t.feature_quantizer.uF.item(),
            "output_scale": model_t.feature_quantizer.output_scale.item(),
            "bkwd_scaling_factorF": model_t.feature_quantizer.bkwd_scaling_factorF.item()
        }

        writer.add_scalar("teacher_quantizer/lF", feature_quantizer_params["lF"], ep)
        writer.add_scalar("teacher_quantizer/uF", feature_quantizer_params["uF"], ep)
        writer.add_scalar("teacher_quantizer/output_scale", feature_quantizer_params["output_scale"], ep)
        writer.add_scalar("teacher_quantizer/bkwd_scaling_factorF", feature_quantizer_params["bkwd_scaling_factorF"], ep)

        # Log details into log.txt
        logging.info(f"Epoch {ep}: Teacher Quantizer Parameters")
        for param, value in feature_quantizer_params.items():
